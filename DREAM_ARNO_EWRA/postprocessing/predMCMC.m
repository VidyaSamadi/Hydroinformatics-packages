function [par_unc,tot_unc] = predMCMC(Pars,RMSE,Func_name,Measurement,MCMCPar,Extra,PredInt,fx);
% Calculates the desired parameter and total prediction uncertainty from the MCMC samples
%
% SYNOPSIS:     [par_unc,tot_unc] = predMCMC(ParSet,RMSE,Func_name,Measurement,MCMCPar,Extra);
%               [par_unc,tot_unc] = predMCMC(ParSet,RMSE,Func_name,Measurement,MCMCPar,Extra,PredInt);
%
% Input:        ParSet = matrix with MCMC samples
%               RMSE = Root Mean Square Error of best solution
%               Func_name = name of the model to be executed
...
if ( isempty(fx) == 0 ),
    ...
    for j = 1:N_Pars,
        ...
        f_handle = eval(['@(x)',char(Func_name),'(x,Extra)']); Spar(:,j) = f_handle(Pars(j,1:MCMCPar.n));            
