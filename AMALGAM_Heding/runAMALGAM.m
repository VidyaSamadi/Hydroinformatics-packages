...
if example == 3,    % Rotated problem
    ...
    ss=1;
    Send=320;
    AMALGAMPar.n = 12;                      % Dimension of the problem
    AMALGAMPar.N = 500;                     % Size of the population
    AMALGAMPar.nobj = 2;                    % Number of objectives
    AMALGAMPar.ndraw =50000;               % Maximum number of function evaluations
    
    % Define the parameter ranges (minimum and maximum values)
    ParRange.minn = ones(1,AMALGAMPar.n); ParRange.maxn = 20*ones(1,AMALGAMPar.n);
    % How is the initial sample created -- Latin Hypercube sampling
    Extra.InitPopulation = 'LHS';
...
plot(ObjVals(:,1),(-100*(ObjVals(:,2))),'*');
xlabel('F1');
ylabel('F2"Reliability"');